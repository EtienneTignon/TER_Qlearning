...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        """
        self.lin1 = nn.Linear(64*9, 200)
        self.bnl1 = nn.BatchNorm1d(200)
        self.lin2 = nn.Linear(200, 150)
        self.bnl2 = nn.BatchNorm1d(150)
        self.lin3 = nn.Linear(150, 9)
        self.bnl3 = nn.BatchNorm1d(9)
...
class DeepQAgent:
    ...
    def _MaJreseau_Batch(self):
        ...
        state_batch = torch.cat(batch.state).view(-1,1,3,3)
        ...
        next_state_values[non_final_mask] = self.target_net(non_final_next_states.view(-1,1,3,3)).max(1)[0].detach()
    ...
    def _MaJreseau(self,recompense,etat,terminal = False):        
        ...
        next_State = torch.tensor(etat,dtype=torch.float)
    ...
    def faireUnChoix(self,etat,tauxHasard=0.0):
        ...
        state = torch.tensor(etat,dtype=torch.float)
        ...
        self.values = self.policy_net(state.unsqueeze(0))
...
#dqa = DeepQLearning(xin=4,yin=4,zin=1,xout=4,yout=4)
dqa = DeepQAgent()
...
while(i<10000):
    ...
    while(j<100):
        ...
        if(disqualification or game.winner==2):
            dqa.perdre()
            defaite += 1
        elif(disqualification2 or game.winner==1):
            dqa.gagner()
            victoire += 1
        else:
            dqa.egaliser()
